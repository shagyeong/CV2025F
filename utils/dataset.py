...
class Dataset(data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.train:
            ...
            # 사용자 정의 증강 모듈 적용
            img = self.randomUserspnoise(img)
            img = self.randomUsershear(img)
    ...
    # 사용자 정의 모듈: 랜덤 salt-pepper noise
    def randomUserspnoise(self,bgr):
        r=bgr.shape[:2][0]             # row size
        c=bgr.shape[:2][1]             # column size
    ...
    def randomUsershear(self,bgr):
        ...
        # 원본 이미지 크기로 다운스케일
        bgr=cv2.resize(bgr,(c,r),interpolation=cv2.INTER_AREA)
        return bgr
